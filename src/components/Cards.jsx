...
const Cards = () => {
  ...
  useEffect(()=>{
    async function getData(){
      ...
      console.log(_data)
      _data.forEach((doc)=>{
        setData((prv)=>[...prv, {...doc.data(), id: doc.id}])            //setData([...data, doc.data()])  
      })
      setLoading(false)
    }
    getData()
        ...
        data.map((e, i) => {
          return (
            <Link to={`/detail/${e.id}`} key={i} >
              <div  className='card font-medium shadow-lg p-2 mt-6 hover:-translate-y-3 transition-all duration-500 cursor-pointer'>
